...
func TestMarshal(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"struct with all values": {
			...
			want: pathEscape("age=30&aliases[]=johnny&aliases[]=j&name=john"),
		},
		...
		"struct with omitempty and non-zero values": {
			...
			want: pathEscape("age=25&aliases[]=j&created_at=0001.01.01&id=0&name=jane"),
		},
		"struct with custom type": {
			...
			want: pathEscape("age=25&aliases[]=janet&aliases[]=jan&created_at=2025.02.08&id=1&name=jane&optional=optional_value"),
		},
		"empty slice": {
			input: &BasicForm{
				Name:    "john",
		...
		"slice with empty strings": {
			...
			want: pathEscape("aliases[]=&aliases[]=&name=john"),
		},
...
func TestMarshal_RoundTrip(t *testing.T) {
	...
	for name, tt := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := formenc.Unmarshal(encoded, tt.target); err != nil {
...
func BenchmarkMarshal(b *testing.B) {
	...
	for name, bm := range benchmarks {
		...
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				_, err := formenc.Marshal(bm.input)
				if err != nil {
