...
func TestInvalidUnmarshalError(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"non-pointer target": {
			target:  BasicForm{},
			wantMsg: "form: Unmarshal(non-pointer formenc_test.BasicForm)",
		},
		"nil pointer target": {
			target:  (*BasicForm)(nil),
			wantMsg: "form: Unmarshal(nil *formenc_test.BasicForm)",
		},
	}
	for name, tt := range tests {
		tt := tt
...
func BenchmarkUnmarshal(b *testing.B) {
	benchmarks := map[string]struct {
		...
		"basic form": {
			input:  []byte("name=john&age=20&aliases[]=johnny&aliases[]=jonny"),
			target: func() interface{} { return &BasicForm{} },
		},
		"complex form": {
			input:  []byte("id=1&name=jane&age=25&aliases[]=janet&aliases[]=jan&created_at=2025.02.08&optional=optional_value"),
			target: func() interface{} { return &ComplexForm{} },
		},
		"nested form": {
			input:  []byte("name=john&age=30&address[street]=123+Main+St&address[city]=Anytown&address[state]=CA&address[zip]=12345"),
			target: func() interface{} { return &NestedForm{} },
		},
		"small map": {
